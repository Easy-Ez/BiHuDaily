...
public class FlowLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        boolean mayHaveSpace = true;
        int lineFreeWidthSpace = 0;
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (lineWidth + childWidth <= freeTotalWidith) {
                lineWidth += childWidth;
                lineHeight = Math.max(lineHeight, childHeight);
                lineFreeWidthSpace = freeTotalWidith - lineWidth;
            } else {
                // 换行
                line++;
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < getRealChildCount(); i++) {
            ...
            if (i == newLineIndexList.get(currentLine)) {
                currentLeft = l + getPaddingLeft();
                curretnTop += curretnBotton;
            }
